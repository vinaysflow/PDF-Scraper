...
- **native_similarity** &lt; 0.90 on page 1 (native text present but very different from OCR) → strict gate fails even when we correctly choose native extraction.
...
3. **Text layout** optionally gets slight relaxation for dual_pass when target is 90, or a dedicated “mixed” handling when native extraction and OCR disagree.
4. **Native-chosen pages**: when we select native and native_similarity is low, optionally **skip or relax** the native_similarity gate (we are not using OCR for that page).
...
- Add a structure keyed by layout (and optionally by quality_target), e.g.:
  ...
  - Each value: `max_low_conf_ratio`, `min_pass_similarity`, `min_native_similarity`, `min_avg_confidence` (only keys that override).
...
- Apply merged gates to the four checks (avg_confidence, low_conf_ratio, dual_pass_similarity, native_similarity).
...
## Phase 2: Native-chosen pages and native_similarity gate

**2.1 When source is native, optionally skip or relax native_similarity**

- In `_page_quality`, after computing `decision` and `selected_source`:
  - If `selected_source == "native"` and `native_similarity is not None` and we have a config flag (e.g. `relax_native_gate_when_native_chosen` or part of layout/quality config):
    - **Option A:** Do not add `"native_similarity"` to `failed_gates` when we chose native (page is “approved” for that gate when native is selected).
    - **Option B:** Use a lower threshold for native_similarity when native is chosen (e.g. 0.5 or 0.0) so we don’t fail the page.

- Prefer **Option A** so that “we trust native for this page” does not fail the document.

**2.2 Config**

- Add a single flag (e.g. in quality_overrides or in a small constants block): `skip_native_similarity_gate_when_native_selected: bool = True` for quality_target 90.

**Deliverable:** Pages where we correctly choose native no longer fail solely on native_similarity.

---

## Phase 3: Layout detection and “mixed” / “diagram” (optional)

**3.1 Optional: “mixed” layout when native vs OCR strongly disagree**

- After OCR and native text are available (in extract, when building quality), if `native_text` and `ocr_text` are both non-empty and `similarity_ratio(native_text, ocr_text) < 0.3`, set a **derived** layout or flag, e.g. `layout_effective = "mixed"` for quality only (do not change OCR).
- In `LAYOUT_QUALITY_OVERRIDES`, add `"mixed"`: e.g. `min_native_similarity = 0.0` (and optionally relaxed dual_pass), so such pages are not failed on native_similarity.
...
**Deliverable (Phase 3):** Optional “mixed” handling so low native–OCR similarity doesn’t fail the page when we still choose native or relax the gate.
...
| 2 | Add “skip or relax native_similarity when selected_source is native” | `app/extract.py` |
| 3 | (Optional) Add “mixed” layout/flag and gate relaxation for low native–OCR similarity | `app/extract.py` |
...
- With **quality_target 90** and layout-specific handling:
  - Pages that **choose native** do not fail on **native_similarity** alone.
...
# Base for target 90: min_pass=0.9, min_native=0.9, max_low=0.6, min_avg=90.
